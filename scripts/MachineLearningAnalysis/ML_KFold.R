...
if (group!="ClinicalData") {
  ...
  rawCountsExpressionData = rawCountsExpressionData[, colnames(xTrain)]
}else{
  clinicalData = extractedData[[1]]
}
...
if (group!="ClinicalData") {
  ...
  xTest = data.frame(t(assay(varianceStabilizingTransformation(ddsTest, blind = FALSE))))
}else{
  xTest <- data.frame(predict(dmy, newdata = clinicalData[, c("Stage", "Bres", "ECOG", "treatment")]))
  #Remove the redundant columns
  xTest <- subset(xTest, select = -c(ECOG.Asymptomatic,treatment.Observation))
}
