...
for (temp in tempFiles) {
  ...
  if (classifierName != "plr") {
    ...
    tempResults$CV = splitString[length(splitString)]
...
#library("ggplot2")
linesDf<-currentResults%>%group_by(Signature_f)%>%summarise(max=max(ROC), median=median(ROC), mean=mean(ROC))
...
d1<- ggplot(currentResults, aes(x = ClassifierName, y = ROC, color = CV))+
  ...
  geom_hline(aes(yintercept=median, linetype=Signature_f), data=linesDf, color="grey")+
...
linesDf<-currentResults%>%group_by(Signature_f)%>%summarise(max=max(Sens), median=median(Sens), mean=mean(Sens))
...
linesDf<-currentResults%>%group_by(Signature_f)%>%summarise(max=max(Spec), median=median(Spec), mean=mean(Spec))
...
for(metric in c("ROC", "Sens", "Spec")){
  ...
  tStatistic2 = t.test(x = currentResults[(currentResults$Signature_f == "Cam_121") & (currentResults$CV == "repeatedcv"), metric], y = currentResults[(currentResults$Signature_f == "Clinical Covariates") & (currentResults$CV == "repeatedcv"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic3 = t.test(x = currentResults[(currentResults$Signature_f == "DecisionDx-\nMelanoma") & (currentResults$CV == "repeatedcv"), metric], y = currentResults[(currentResults$Signature_f == "Clinical Covariates") & (currentResults$CV == "repeatedcv"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic4 = t.test(x = currentResults[(currentResults$Signature_f == "LMC_150") & (currentResults$CV == "repeatedcv"), metric], y = currentResults[(currentResults$Signature_f == "Clinical Covariates") & (currentResults$CV == "repeatedcv"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic5 = t.test(x = currentResults[(currentResults$Signature_f == "Cam_121") & (currentResults$CV == "repeatedcv"), metric], y = currentResults[(currentResults$Signature_f == "DecisionDx-\nMelanoma") & (currentResults$CV == "repeatedcv"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic6 = t.test(x = currentResults[(currentResults$Signature_f == "Cam_121") & (currentResults$CV == "repeatedcv"), metric], y = currentResults[(currentResults$Signature_f == "LMC_150") & (currentResults$CV == "repeatedcv"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
...
for(metric in c("ROC", "Sens", "Spec")){
  tStatistic1 = t.test(x = currentResults[currentResults$Signature_f == "Cam_121+\nClinical Covariates", metric], y = currentResults[currentResults$Signature_f == "Clinical Covariates", metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic2 = t.test(x = currentResults[(currentResults$Signature_f == "Cam_121") & (currentResults$CV == "boot"), metric], y = currentResults[(currentResults$Signature_f == "Clinical Covariates") & (currentResults$CV == "boot"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic3 = t.test(x = currentResults[(currentResults$Signature_f == "DecisionDx-\nMelanoma") & (currentResults$CV == "boot"), metric], y = currentResults[(currentResults$Signature_f == "Clinical Covariates") & (currentResults$CV == "boot"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic4 = t.test(x = currentResults[(currentResults$Signature_f == "LMC_150") & (currentResults$CV == "boot"), metric], y = currentResults[(currentResults$Signature_f == "Clinical Covariates") & (currentResults$CV == "boot"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic5 = t.test(x = currentResults[(currentResults$Signature_f == "Cam_121") & (currentResults$CV == "boot"), metric], y = currentResults[(currentResults$Signature_f == "DecisionDx-\nMelanoma") & (currentResults$CV == "boot"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
  tStatistic6 = t.test(x = currentResults[(currentResults$Signature_f == "Cam_121") & (currentResults$CV == "boot"), metric], y = currentResults[(currentResults$Signature_f == "LMC_150") & (currentResults$CV == "boot"), metric], var.equal = FALSE, paired = FALSE, alternative = "greater")
...
write.table(tStatistics, "~/Desktop/Melanoma/train_boot_PVal.tsv", sep = "\t", col.names = TRUE, row.names = TRUE, quote = FALSE)